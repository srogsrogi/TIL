...
* 로지스틱 회귀
  ...
  * sigmoid 함수의 출력값이 0.5 이상이면 1, 아니면 0으로 분류하는 이진 분류를 수행
...
- 대표적인 클러스터링 기법
  - K-Means
    ...
    - 군집의 수를 하이퍼파라미터로 미리 지정해야 하며, 초기 설정에 따라 결과가 달라질 수 있어 불안정

  - DBSCAN
    - 데이터가 몰려 있는 곳은 군집, 아닌 부분은 노이즈로 간주하는 **밀도 기반 클러스터링**
    - 한 점을 기준으로 하는 반경(반지름)과 군집으로 판정하기 위해 반경 내에 있어야 하는 데이터포인트의 수를 하이퍼파라미터로 하여 군집 탐색
...
* 활성화 함수(Activation Function)
  ...
  * 은닉층에서는 출력값을 비선형 변환하여 복잡한 관계를 학습하게 함
  * 출력층에서는 task에 따라 적절한 출력값으로 변환해주는 역할(이진 분류, 다중 분류, 회귀 등)

* 주요 활성화 함수

  * Sigmoid
    * 출력을 0~1로 제한하여 **확률 해석**에 유용
    * 확률 예측 및 이진 분류(0.5 이상이면 True)에 적합
    * 양 극단에서 기울기가 0으로 수렴하여 **기울기 소실** 문제가 발생하여 은닉층에서는 거의 사용되지 않음
    * **이진 분류** task에서 출력층에서 활용
  * Softmax
    * 출력을 확률 분포로 변환
    * 클래스별 확률값을 반환하며 모두 더하면 1인 형태
    * **다중 분류** task에서 출력층에서 활용
  * ReLU
    * 은닉층에서 학습용으로 가장 많이 활용되는 함수
    * 계산이 단순하여 학습이 빠르면서도 기울기 소실 문제가 완화되어 성능이 좋음
    * 음수 입력을 모두 0으로 처리하여 **Dead ReLU** 문제가 발생할 수 있음
    * Leaky ReLU 등 변형된 활성화함수를 사용하여 개선 가능
  * 항등함수 `y=x`
    * 입력층에서 최초 입력값을 그대로 은닉층에 전달하기 위해 사용
    * **회귀** task에서 출력층에서 활용
...
* 역전파(Backpropagation)
  ...
  * 각 가중치가 순전파를 통해 계산된 예측 오차(Loss)에 얼만큼 기여했는지를 계산하는 방법
...
- 내용이 굉장히 많고, 개념들 간의 관계를 한 번에 파악하기 어려울 수 있습니다. 개념 자체도 반복해서 공부하시고, 실제 모델을 활용하여 개발하는 과정 중에도 한 번씩 관련 개념들을 살펴 가며 머릿속에 통합된 하나의 흐름을 만들어 내시길 바랍니다.
- 내용이 많다고는 했지만.. 이 정도는 알아야 다음 단계로 넘어가 주요 모델별 메커니즘이나 특징, 활용법 등을 이해하고 학습할 수 있습니다! 시간을 들여 복습하고 그걸 기반으로 SSAFY 정규 강의도 더 잘 이해해봅시다.

- 시간상 주요 딥러닝 아키텍쳐(5장)에 대한 설명을 생략하거나 크게 압축하여 설명했으니 추가 학습이 필요합니다. 특히 Attention과 Transformer는 NLP 뿐만 아니라 거의 모든 task에서 범용적으로 활용되는 추세이므로, 메커니즘과 장점, 한계 등을 깊이 있게 학습해보세요.