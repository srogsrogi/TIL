...
##  5. 분야별 핵심 딥러닝 아키텍처
   ...
   * 이미지 처리를 위한 CNN (Convolutional Neural Network)
   * 순차 데이터(시계열, 텍스트)를 위한 RNN & LSTM
...
- 내용이 많다고는 했지만.. 이 정도는 알아야 다음 단계로 넘어가 주요 모델별 메커니즘이나 특징, 활용법 등을 이해하고 학습할 수 있습니다! 시간을 들여 복습하고 그걸 기반으로 SSAFY 정규 강의도 이해해봅시다.