...
- 브라우저에 저장된 무언가와 관련이 있을 것 같아서

- 헤드풀 모드(`headless=False`)로 한 번 돌렸더니 즉시 성공
  - 이후 다시 헤드리스로 되돌려도 성공 지속.

- 처음엔 내가 헤드풀모드에서 뭐 허용 누른 것 때문에 되는 줄 알았는데
- `Default/Preferences` JSON을 뒤져본 결과 세움터 관련 키/프로토콜 핸들러 기록 없음. 이 가설도 틀림
...
- incognito는 처음부터 EAISP를 못 갖는 구조인데 이 기능 개발하고 한 2개월 간 문제 없었음
- 아마 세움터가 문제 발생 당일 서버사이드 플래그를 켜서 EAISP 보유 여부를 확인하도록 수정한 것으로 추정
  - 어차피 클라이언트 단에서 더 이상 알 수 있는 방법이 없음
